...
export default function ProfileForm() {
        ...
        return <motion.div key="basics" initial={{
          ...
          x: -50
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-primary-foreground">Tell us a bit about yourself</p>
            </div>

            <div className="space-y-4">
              <div className="text-primary-foreground">
                <Label htmlFor="name">Your name</Label>
                <Input id="name" placeholder="What should we call you?" value={formData.name} onChange={e => updateFormData("name", e.target.value)} className="bg-background/50 border-border/50 h-12" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-primary-foreground">
                ...
                <div>
                  ...
                  <Input id="city" placeholder="Mumbai" value={formData.city} onChange={e => updateFormData("city", e.target.value)} className="bg-background/50 border-border/50 h-12" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {GENDERS.map(g => <Button key={g} type="button" variant={formData.gender === g ? "default" : "outline"} onClick={() => updateFormData("gender", g)} className={formData.gender === g ? "bg-primary" : "border-border/50"}>
                      {g}
                    </Button>)}
                </div>
              </div>
        ...
        return <motion.div key="preferences" initial={{
          ...
          x: -50
            ...
            <div className="space-y-6">
              <div>
                <Label>My orientation is...</Label>
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {ORIENTATIONS.map(o => <Button key={o} type="button" variant={formData.orientation === o ? "default" : "outline"} onClick={() => updateFormData("orientation", o)} className={formData.orientation === o ? "bg-primary" : "border-border/50"}>
                      {o}
                    </Button>)}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                  {SHOW_ME_OPTIONS.map(o => <Button key={o} type="button" variant={formData.showMe.includes(o) ? "default" : "outline"} onClick={() => toggleShowMe(o)} className={formData.showMe.includes(o) ? "bg-primary" : "border-border/50"}>
                      {o}
                    </Button>)}
                </div>
              </div>
  ...
  return <div className="min-h-screen flex flex-col p-6">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-primary/10 rounded-full blur-[100px]" />
      </div>
