...
### User flow
1. `/` — Landing page; enter an event code to join or create an event
2. `/join` — Join an existing event by code
...
### Session management
Participants are identified by a `session_token` (UUID) stored in `localStorage` via `src/lib/session.ts`. There is no user account system. The token is saved when the participant submits the profile form and checked on every protected route to redirect appropriately.
...
- Env vars required: `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY`
...
- `FormStep` — union type driving the multi-step profile form
